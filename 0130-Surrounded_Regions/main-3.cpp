...
public:
   void solve(vector<vector<char>>& board) {
       ...
       for (int i = 0; i < row; i++) {
           if (board[i][0] == 'O')
               uf.parent[i * row] = dummy;
           if (board[i][col - 1] == 'O')
               uf.parent[i * row + (col - 1) % col] = dummy;
       }
       for (int j = 0; j < col; j++) {
           if (board[0][j] == 'O')
               uf.parent[j % col] = dummy;
           if (board[row - 1][j] == 'O')
               uf.parent[(row - 1) * row + j % col] = dummy;
       }
       // 用并查集遍历全部点
       for (int i = 0; i < row; i++)
           for (int j = 0; j < col; j++) {
               if (board[i][j] == 'O') {
                   for (int d = 0; d < 4; d++) {
                       ...
                       if (inArea(x, y))
                           ...
                           unionElm(x * row + y % col, i * row + j % col, uf);
                   }
               }
       ...
       for (int i = 0; i < row; i++)
           for (int j = 0; j < col; j++) {
               if (board[i][j] == 'O' && uf.parent[i * row + j % col] != dummy)
                   board[i][j] = 'X';
           }
        return;  
   }
